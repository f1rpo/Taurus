...
/*	<trs.modname> I'm guessing that the string structure used by the EXE for storing
	the mod name is the mysterious FString class (F for "Firaxis Game Engine")
	mentioned in comments in CvString.h. */
class FString
{
	FString(); // Will get our instances only from the EXE
	...
	/*	It's an array. Don't know how that gets allocated locally like this
		...
		is allocated on the heap and then reinterpret-cast. */
	char m_cFirstChar;
	...
	//char const* getCString() const { return m_szBuffer; }
public:
	// The EXE will provide a C string, like in the comment above.
	static FString* create(char const* szExternal)
	{
		if (szExternal == NULL)
		{
			FAssert(szExternal != NULL);
			return NULL;
		}
...
class ModNamePatch : public RuntimePatch
{
public:
	ModNamePatch(ModName const& kModName) : m_kModName(kModName) {}
	/*	Not actually self-modifying, just overwrites strings in the data section.
		Don't need to change access permissions for that, and doesn't depend
		on specific virtual memory addresses either. */
	bool isOptionalThroughXML() const { return false; } // override
protected:
	bool apply() // override
	{
		FString* pPathInRoot = FString::create(m_kModName.getPathInRootExt());
		FString* pFullPath = FString::create(m_kModName.getFullPathExt());
		if (pPathInRoot == NULL || pFullPath == NULL)
			return false;
		if (!pPathInRoot->assign(m_kModName.getPathInRoot()) ||
			!pFullPath->assign(m_kModName.getFullPath()))
		{
			return false;
		}
		return true;
...
// <trs.modname>
// Wrapper, to keep bRestore out of the public interface.
void Civ4BeyondSwordMods::setExternalModName(char const* szName)
{
	setExternalModName(szName, false);
}


void Civ4BeyondSwordMods::setExternalModName(char const* szName, bool bRestore)
{
	if (gDLL == NULL || !GC.isCachingDone())
	{
		FErrorMsg("Mustn't change mod name while still loading files");
		return;
	}
	m_externalModName = GC.getModName(); // true name
	m_externalModName.replaceName(szName); // desired name
	bool bSuccess = true;
	if (std::max(
		std::strlen(m_externalModName.getPathInRoot()),
		// I've seen capacity 31 in the debugger, not counting the terminating \0.
		std::strlen(m_externalModName.getFullPath())) > 31)
	{
		FErrorMsg("Likely not enough capacity for replacement name");
		bSuccess = false;
	}
	ModNamePatch patch(m_externalModName);
	bSuccess = (bSuccess && patch.applyIfEnabled() &&
			// EXE will return NULL if size is 0 (not an empty string)
			((gDLL->getModName(false) == NULL || gDLL->getModName(true) == NULL) ?
			m_externalModName.getName()[0] == '\0' :
			(std::strcmp(
			gDLL->getModName(false),
			m_externalModName.getPathInRoot()) == 0 &&
			std::strcmp(
			gDLL->getModName(true),
			m_externalModName.getFullPath()) == 0)));
	if (bSuccess)
		return;
	FErrorMsg("Failed to replace mod name");
	// Try once (not in an infinite loop) to restore the true name.
	if (!bRestore)
	{
		m_externalModName = GC.getModName();
		setExternalModName(m_externalModName.getName(), true);
	}
} // </trs.modname>
